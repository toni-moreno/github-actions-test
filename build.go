...
var (
	...
	race                  bool   = false
	workingDir            string
	serverBinaryName      string = "github-actions-test"
)
...
func makeLatestDistCopies() {
	...
	runError("cp", "dist/github-actions-test_"+version+"_amd64.deb", "dist/github-actions-test_latest_amd64.deb")
	runError("cp", "dist/github-actions-test-"+linuxPackageVersion+rpmIteration+".x86_64.rpm", "dist/github-actions-test-latest-1.x86_64.rpm")
	runError("cp", "dist/github-actions-test-"+version+".linux-x64.tar.gz", "dist/github-actions-test-latest.linux-x64.tar.gz")
}
...
func readVersionFromGit() {
	...
	linuxPackageVersion = strings.TrimLeft(linuxPackageVersion,"v")
...
func createDebPackages() {
	createFpmPackage(linuxPackageOptions{
		packageType:            "deb",
		homeDir:                "/usr/share/github-actions-test",
		binPath:                "/usr/sbin/github-actions-test",
		configDir:              "/etc/github-actions-test",
		configFilePath:         "/etc/github-actions-test/config.toml",
		etcDefaultPath:         "/etc/default",
		etcDefaultFilePath:     "/etc/default/github-actions-test",
		initdScriptFilePath:    "/etc/init.d/github-actions-test",
		systemdServiceFilePath: "/usr/lib/systemd/system/github-actions-test.service",

		postinstSrc:    "packaging/deb/control/postinst",
		initdScriptSrc: "packaging/deb/init.d/github-actions-test",
		defaultFileSrc: "packaging/deb/default/github-actions-test",
		systemdFileSrc: "packaging/deb/systemd/github-actions-test.service",
...
func createRpmPackages() {
	createFpmPackage(linuxPackageOptions{
		...
		etcDefaultFilePath:     "/etc/sysconfig/github-actions-test",
		initdScriptFilePath:    "/etc/init.d/github-actions-test",
		systemdServiceFilePath: "/usr/lib/systemd/system/github-actions-test.service",

		postinstSrc:    "packaging/rpm/control/postinst",
		initdScriptSrc: "packaging/rpm/init.d/github-actions-test",
		defaultFileSrc: "packaging/rpm/sysconfig/github-actions-test",
		systemdFileSrc: "packaging/rpm/systemd/github-actions-test.service",
...
func createMinTar() {
	packageRoot, _ := ioutil.TempDir("", "github-actions-test-linux-pack")
	// create directories
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/github-actions-test"))
...
//	runPrint("cp", "-a", filepath.Join(workingDir+"/public"), filepath.Join(packageRoot, options.homeDir))
	...
	args := []string{
		...
		"-C", packageRoot,
		"--vendor", "github-actions-test",
		"--url", "http://github.org/toni-moreno/github-actions-test",
		"--license", "Apache 2.0",
...
func ldflags(flags []string) string {
	var b bytes.Buffer
	b.WriteString("-w")
	b.WriteString(fmt.Sprintf(" -X github.com/toni-moreno/github-actions-test/pkg/agent.Version=%s", version))
	b.WriteString(fmt.Sprintf(" -X github.com/toni-moreno/github-actions-test/pkg/agent.Commit=%s", getGitSha()))
	b.WriteString(fmt.Sprintf(" -X github.com/toni-moreno/github-actions-test/pkg/agent.BuildStamp=%d", buildStamp()))
	for _, f := range flags {
